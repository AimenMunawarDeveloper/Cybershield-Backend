...
// GET /api/admins/email-learning-summary - platform-wide learning score (email) for system_admin; higher = better
const getEmailLearningSummary = async (req, res) => {
  try {
    const { limit = 100 } = req.query;
    const users = await User.find({ role: { $in: ['affiliated', 'non_affiliated'] } })
      .select('_id displayName email learningScoreEmail role')
      .sort({ learningScoreEmail: 1 })
      .limit(Number(limit))
      .lean();
    const withScore = users.filter(u => u.learningScoreEmail != null);
    const sum = withScore.reduce((a, u) => a + (u.learningScoreEmail ?? 0), 0);
    const averageLearningScoreEmail = withScore.length > 0 ? sum / withScore.length : 0;
    res.json({
      success: true,
      data: {
        averageLearningScoreEmail: Math.round(averageLearningScoreEmail * 100) / 100,
        totalUsers: users.length,
        users: users.map(u => ({
          _id: u._id,
          displayName: u.displayName,
          email: u.email,
          learningScoreEmail: u.learningScoreEmail != null ? u.learningScoreEmail : 0,
          role: u.role
        }))
      }
    });
  } catch (error) {
    console.error('Email learning summary error:', error);
    res.status(500).json({ success: false, error: 'Failed to fetch email learning summary' });
  }
};
